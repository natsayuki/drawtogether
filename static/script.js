...
function draw(){
  socket.emit('draw', {'mouseX': mouseX, 'mouseY': mouseY, 'drawing': drawing, 'color': toggle, 'shape': shape, 'size': size});
  socket.on('returnDraw', (data)=>{
    mouseX2 = data['mouseX']; mouseY2 = data['mouseY']; drawing = data['drawing']; color = data['color']; shape = data['shape']; size = data['size'];
    // fill(color)
    if(drawing){
      if(shape == 'rect'){
        ...
        ellipse(mouseX2, mouseY2, size, size);
      }
    }
  });
}
